...
def lista_produtos(request):
    ...
    return render(request, "meuapp/lista_produtos.html", {"itens": itens})
...
def novo_item(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():  # Verifica se o formulário é válido
            ...
            return redirect("lista_produtos")  # Redireciona para a lista de itens
    else:
        form = NovoItemForm()  # Cria um formulário vazio (para ser preenchido pelo usuário)

    return render(request, "meuapp/novo_item.html", {"form": form})
...
def adicionar_item(request):
    ...
    return render(request, "meuapp/adicionar_item.html", {"form": form})
...
def detalhes_item(request, id):
    ...
    return render(request, "meuapp/detalhes_item.html", {"item": item})
...
def editar_item(request, id):
    ...
    return render(request, "meuapp/editar_item.html", {"form": form, "item": item})
...
def confirmar_exclusao_item(request, id):
    ...
    """
    item = get_object_or_404(ItemEstoque, id=id)  # Obtém o item com o ID fornecido
    return render(request, "meuapp/confirmar_exclusao_item.html", {"item": item})
